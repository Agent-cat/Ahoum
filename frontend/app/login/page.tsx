...
export default function LoginPage() {
  ...
  useEffect(() => {
    Promise.allSettled([
      fetch("/api/auth/github/url/").then((r) => (r.ok ? r.json() : Promise.reject())),
      fetch("/api/auth/google/url/").then((r) => (r.ok ? r.json() : Promise.reject())),
      fetch("/api/auth/dev-login/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: "{}",
      }).then((r) => (r.ok ? true : Promise.reject())),
    ]).then(([github, google, dev]) => {
      if (github.status === "fulfilled") {
        setGithubUrl(github.value.authorize_url);
        localStorage.setItem("oauth_state_url", github.value.authorize_url);
      }
      if (google.status === "fulfilled") {
        setGoogleUrl(google.value.authorize_url);
      }
